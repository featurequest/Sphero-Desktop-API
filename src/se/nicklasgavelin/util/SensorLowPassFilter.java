...
public class SensorLowPassFilter extends SensorFilter
{
  ...
  public void addDatum(double x, double y, double z)
  {
    ...
    if (this.adaptive)
    {
      float d = (float)Value.clamp(Math.abs(Vector3D.magnitude(this.x, this.y, this.z) - Vector3D.magnitude(x, y, z)) / 0.01999999955296516D - 1.0D, 0.0D, 1.0D);

      alpha = (1.0F - d) * this.filterConstant / 3.0F + d * this.filterConstant;
    }
