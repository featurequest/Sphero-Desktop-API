package site.nicklas.bluetooth;
...
public class BluetoothDevice implements DiscoveryListener
{
        ...
        if ( this.activeSearch < 0 )
        {
            try
            {
                ...
                synchronized ( this.bt )
                {
                    ...
                    this.activeSearch = this.bt.getDiscoveryAgent().searchServices(
                            new int[]
                            {
                                BluetoothService.ATTR_SERVICENAME,
                                BluetoothService.ATTR_SERVICEDESC,
                                BluetoothService.ATTR_PROVIDERNAME
                            },
                            new javax.bluetooth.UUID[]
                            {
                                this.bt.getUUID()
                            },
                            this.device,
    ...
    public void servicesDiscovered( int transId, ServiceRecord[] records )
    {
        ...
        {
            if ( records.length > 0 )
                this.service = new BluetoothService( this, records[0], this.bt );// TODO: Will there ever be multiple services???
        }
    }
